...
print('----------------------------------------------------')
# scan with pool of processes
ret = []

def callback(args):
    host, scan_data = args
    if scan_data['scan'] and \
       scan_data['scan']['status']['state'] == "up":
        ret.append(host)

nmp = nmap.PortScannerPool(processes=5)
nm.scan('127.0.0.1/24', '22-25', callback=callback)
nmp.wait()
print(ret)
